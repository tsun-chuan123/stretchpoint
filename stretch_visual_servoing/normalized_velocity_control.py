...
class NormalizedVelocityControl():
    ...
    def stop(self):
        with self.lock:
            ...
            try:
                ...
                # If hardware is not ready or another process held the bus,
    ...
    def get_joint_state(self):
        with self.lock:
            ...
            # Head joints may be unavailable if the robot isn't homed/connected yet
            head_pan_dict = self.robot.head.status.get('head_pan') if hasattr(self.robot.head, 'status') else None
            head_tilt_dict = self.robot.head.status.get('head_tilt') if hasattr(self.robot.head, 'status') else None
            head_pan_pos = head_pan_dict.get('pos') if isinstance(head_pan_dict, dict) else None
            head_pan_eff = head_pan_dict.get('effort') if isinstance(head_pan_dict, dict) else None

            head_tilt_pos = head_tilt_dict.get('pos') if isinstance(head_tilt_dict, dict) else None
            head_tilt_eff = head_tilt_dict.get('effort') if isinstance(head_tilt_dict, dict) else None
    ...
    def controller_loop(self):
        while True: 
            with self.lock:
                if self.stop_loop:
                    # Cleanly exit the controller thread without raising SystemExit
                    return
