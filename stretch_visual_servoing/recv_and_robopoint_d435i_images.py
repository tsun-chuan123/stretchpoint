...
# Networking for D435i stream (align with recv_and_yolo_d405_images)
import zmq
...
class YoloGraspProcess:
    ...
    def start(self, use_remote: bool = False):
        ...
        # cmd += ["--remote-ip", yn.remote_computer_ip, "--d435i-port", str(yn.d435i_port)]
...
class ImageLabel(QLabel):
    ...
    def __init__(self):
        ...
        self.setText("Waiting for D435i frames…")
...
class D435iReceiverThread(QThread):
    """Background thread mirroring recv_and_yolo_d405_images subscription to receive D435i frames."""
    ...
    def run(self):
        try:
            ...
            if self.use_remote:
                address = 'tcp://' + yn.robot_ip + ':' + str(yn.d435i_port)
            else:
            ...
            while self._running:
                try:
                    d435i_output = self._socket.recv_pyobj(flags=0)
                except zmq.error.ZMQError:
                    break
                if not self._running:
                    break
                if isinstance(d435i_output, dict):
                    self.frame_received.emit(d435i_output)
        except Exception as e:
...
class YoloInlineThread(QThread):
    """Run YOLO perception inline by subscribing to D435i frames and emitting results.
    ...
    def run(self):
        try:
            ...
            # ZMQ SUB to D435i stream (aligned with existing networking options)
            ctx = zmq.Context()
            ...
            if self.use_remote:
                d435i_addr = 'tcp://' + yn.robot_ip + ':' + str(yn.d435i_port)
                model_name = yn.yolo_model_on_remote_computer
            else:
                d435i_addr = 'tcp://' + '127.0.0.1' + ':' + str(yn.d435i_port)
                model_name = yn.yolo_model_on_robot
            sub.connect(d435i_addr)
            self.status_changed.emit(f"YOLO Inline Connected ({d435i_addr})")
            ...
            while self._running:
                ...
                try:
                    d435i_output = sub.recv_pyobj(flags=zmq.NOBLOCK)
                except zmq.Again:
                    self.msleep(1)
                    continue

                color_image = d435i_output.get('color_image')
                depth_image = d435i_output.get('depth_image')
                depth_camera_info = d435i_output.get('depth_camera_info')
                depth_scale = d435i_output.get('depth_scale')
...
class RoboPointMainWindow(QMainWindow):
    ...
    def __init__(self, controller_url=None, autostart=True, model_path="wentao-yuan/robopoint-v1-vicuna-v1.5-13b", load_4bit=True, use_remote_d435i=False):
        super().__init__()
        ...
        self.use_remote_d435i = use_remote_d435i
        # Assume YOLO runs on the remote computer when D435i is remote.
        self.use_remote_yolo = use_remote_d435i
        self.d435i_thread = None
        ...
        try:
            ...
            from stretch_visual_servoing import d435i_helpers_without_pyrealsense as dh
            self.dh = dh
    ...
    def setup_ui(self):
        ...
        # D435i stream area (replaces upload/camera controls)
        image_group = QGroupBox("D435i Stream")
        image_layout = QVBoxLayout(image_group)
        self.image_label = ImageLabel()
        image_layout.addWidget(self.image_label)
        # Status label for stream
        self.d435i_status = QLabel("Status: Disconnected")
        image_layout.addWidget(self.d435i_status)
        ...
        # Start D435i receiver thread
        self.start_d435i_receiver()
    ...
    def start_d435i_receiver(self):
        """Start background thread to receive D435i frames via ZMQ and update preview."""
        if self.d435i_thread is not None:
            return
        self.d435i_thread = D435iReceiverThread(use_remote=self.use_remote_d435i)
        self.d435i_thread.frame_received.connect(self.on_d435i_frame)
        # Also feed frames to ArUco detection slot
        self.d435i_thread.frame_received.connect(self.process_aruco_from_d435i)
        self.d435i_thread.status_changed.connect(self.on_d435i_status)
        self.d435i_thread.start()
    ...
    def on_d435i_status(self, text):
        self.d435i_status.setText(f"Status: {text}")

    def on_d435i_frame(self, d435i_output):
        """Handle incoming D435i frame dict: display color image in the preview label."""
        try:
            color_image = d435i_output.get('color_image', None)
            depth_image = d435i_output.get('depth_image', None)
            if color_image is None:
                return
            # Cache camera parameters for downstream computations
            self.latest_camera_info = d435i_output.get('depth_camera_info', d435i_output.get('color_camera_info', None))
            self.latest_depth_scale = d435i_output.get('depth_scale', None)
            ...
            logger.error(f"Failed to handle D435i frame: {e}")

    def process_aruco_from_d435i(self, d435i_output):
        """Run ArUco detection from incoming D435i frame and cache fingertips for overlay."""
        try:
            color_image = d435i_output.get('color_image', None)
            if color_image is None:
                return
            camera_info = d435i_output.get('depth_camera_info', d435i_output.get('color_camera_info', None))
    ...
    def process_response_visualization(self, response):
        ...
        try:
            ...
            # Cache points for live overlay on D435i frames
            try:
                w_vis, h_vis = 640, 480
                pts = []
    ...
    # Camera-related functions removed; replaced by D435i stream receiver
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            if self.d435i_thread is not None:
                self.d435i_thread.stop()
                self.d435i_thread.wait(1000)
            if self.yolo_thread is not None:
...
def main():
    ...
    parser.add_argument('-r', '--remote', action='store_true', help='Use when receiving D435i images from a remote robot. Mirrors recv_and_yolo_d405_images.')
